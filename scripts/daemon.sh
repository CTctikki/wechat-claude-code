...
# =============================================================================
# Windows (MINGW/MSYS/Git Bash) functions
# =============================================================================
...
windows_start() {
  ...
  local node_bin="$(command -v node || echo 'node')"
  ...
  echo "Starting wechat-claude-code daemon (Windows background)..."
  nohup "$node_bin" "${PROJECT_DIR}/dist/main.js" start \
    >> "$DATA_DIR/logs/stdout.log" \
    2>> "$DATA_DIR/logs/stderr.log" &
  local pid=$!
...
main() {
  ...
  case "$OS_TYPE" in
    ...
    Linux)
      ...
      ;;
    *)
      # Windows (MINGW, MSYS, Git Bash, Cygwin)
      case "$command" in
        start)   windows_start ;;
        stop)    windows_stop ;;
        ...
        *)
          echo "Usage: daemon.sh {start|stop|restart|status|logs}"
          echo "Platform: Windows (background process)"
          exit 1
          ;;
      esac
      ;;
