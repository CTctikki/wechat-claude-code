...
微信官方提供了 [WeChat MCP](https://github.com/anthropics/claude-code/issues) 插件，基于 MCP 协议让 Claude 读取微信消息。以下是两者的对比：

| 功能 | 本项目 (wechat-claude-code) | 微信官方 MCP 插件 |
|------|:---:|:---:|
| **对话方式** | 微信直接聊天，手机随时用 | 需在电脑端 Claude Code 中操作 |
| **图片识别** | ✅ 自动下载解密 + 文件传递 | ❌ 不支持 |
| **语音消息** | ✅ 自动语音转文字 | ❌ 不支持 |
| **文件处理** | ✅ 自动下载解密 | ❌ 不支持 |
| **实时进度** | ✅ 工具调用实时推送到微信 | ❌ 无（结果在电脑端显示） |
| **思考预览** | ✅ 推理摘要实时推送 | ❌ 无 |
| **中断/重定向** | ✅ 发消息即可打断 | ❌ 不支持 |
| **权限审批** | ✅ 微信内交互式审批 | ❌ 需在电脑端操作 |
| **斜杠命令** | ✅ 14+ 命令 | ❌ 无 |
| **Skill 触发** | ✅ 微信中触发任意 Skill | ❌ 不支持 |
| **系统提示词** | ✅ `/prompt` 持久化设置 | ❌ 不支持 |
| **会话管理** | ✅ 持久化 + 压缩 + 撤销 | ❌ 无独立会话 |
| **后台运行** | ✅ 守护进程，开机自启 | 需手动启动 |
| **跨平台** | ✅ macOS / Linux / Windows | 仅 Windows/macOS 桌面端 |
| **移动端使用** | ✅ 手机微信即可 | ❌ 必须在电脑前 |
| **多账号** | ✅ 支持 | ❌ 单账号 |
| **协议** | Claude Code Agent SDK | MCP 协议 |
| **读取联系人/群聊** | ❌ 不支持 | ✅ 支持 |
| **发送消息给他人** | ❌ 不支持 | ✅ 支持 |
| **读取历史消息** | ❌ 不支持 | ✅ 支持 |

**总结：** 本项目专注于"用微信和 Claude 对话"的场景，提供完整的移动端体验；官方 MCP 插件则侧重于让 Claude 读写微信数据（联系人、群聊、历史消息），适合在电脑端进行微信数据分析。两者定位不同，互为补充。
